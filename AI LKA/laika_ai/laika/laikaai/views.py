...
@csrf_exempt
def email_assistant(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        command = data.get('command', '').lower()
        if 'one' in command:
            response_text = 'Composing a new email.'
        elif 'two' in command:
            response_text = 'Checking your inbox.'
        elif 'three' in command:
            response_text = 'Deleting the most recent email.'
        else:
            response_text = (
                "Welcome to the voice-based email system. "
                "Say one for compose email, "
                "say two for check inbox, "
                "say three for delete recent email."
            )
        speak(response_text)
        return JsonResponse({'response': response_text})
    
    initial_response = (
        "Welcome to the voice-based email system. How can I help you? "
        "Say one for compose email, "
        "say two for check inbox, "
        "say three for delete recent email."
    )
...
def get_current_time_view(request):
    ...
    return JsonResponse({'time': f"The current time is {current_time}"})
...
def process_command(request):
    ...
    if request.method == 'POST':
        ...
        if 'current time' in command:
            ...
            youtube_search_url = f"https://www.youtube.com/results?search_query={song_name}"
            return JsonResponse({'response': youtube_search_url})
        
        else:
            return JsonResponse({'response': "Sorry, I didn't catch that. Please say it again."})
